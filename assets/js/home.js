 ...
 ***********************************************************/
		...
		postList = document.querySelector('.loop-archive__inner');
		...
		function toLoadMore() {
			btnLoad.addEventListener('click', () => {
				noLoadNextPage();
				for (const post of nextDom.querySelectorAll('.loop-archive > div > .loop-archive__item')) {
...
if (document.querySelector('.loop-source__items')) {
  var slider = new Glider(document.querySelector('.loop-source__items'), {
      slidesToScroll: 1,
      slidesToShow: 1,
      draggable: true,
      rewind: true,
  ...
  document.querySelector('.loop-source__items').addEventListener('mouseover', (event) => {
      if (autoplay != null) {
  ...
  document.querySelector('.loop-source__items').addEventListener('mouseout', (event) => {
      if (autoplay == null) {
  ...
  /* view front page toggle*/
  let frontToggle = document.querySelector('.front-header__view');

  frontToggle.addEventListener('click', e => {
    e.preventDefault();
    document.documentElement.setAttribute('front-page', frontState)
    frontState = (frontState === 'archive' ? 'section' : 'archive');
    localStorage.setItem('front-state', frontState);
    document.documentElement.setAttribute('front-page', frontState)
